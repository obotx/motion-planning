...
class ParallelRobot:
    ...
    def control_arm(self, target_arm_id):
        ...
        if self.success :
            if (self.grab_state == "trajectory" and self.current_waypoint_idx > 0) :
                self.data.ctrl[self.actuator_ids[1:]] = np.array([0, 0, 0, 0])
            else :
                self.data.ctrl[self.actuator_ids[1:]] = act_clipped

    def control_base_trajectory(self, alpha=0):
        k_p = 5.0
        k_i = 0.1
        k_d = 0.8
        k_p_theta = 5.0
        ...
        if self.grab_state == "trajectory" and self.current_waypoint_idx < len(self.base_trajectory) - 1:
            start_x, start_y, start_yaw = self.base_trajectory[self.current_waypoint_idx]
            end_x, end_y, end_yaw = self.base_trajectory[self.current_waypoint_idx + 1]
            
            distance = np.sqrt((end_x - start_x)**2 + (end_y - start_y)**2)
            time_to_next = distance / self.speed if self.speed > 0 else 1.0
            angular_distance = np.abs(end_yaw - start_yaw)
            angular_time = angular_distance / self.angular_speed if self.angular_speed > 0 else 1.0
            total_time = max(time_to_next, angular_time)
            
            self.time_at_current_waypoint += self.model.opt.timestep
            self.progress = min(self.time_at_current_waypoint / total_time, 1.0)
            
            target_x = start_x + (end_x - start_x) * self.progress
            target_y = start_y + (end_y - start_y) * self.progress
            target_yaw = start_yaw + (end_yaw - start_yaw) * self.progress
            
            if self.progress >= 1.0:
                self.current_waypoint_idx += 1
                self.time_at_current_waypoint = 0.0
        elif self.grab_state == "trajectory2" and self.current_waypoint_idx < len(self.base_trajectory2) - 1:
            start_x, start_y, start_yaw = self.base_trajectory2[self.current_waypoint_idx]
            end_x, end_y, end_yaw = self.base_trajectory2[self.current_waypoint_idx + 1]
            ...
            if self.progress >= 1.0:
                self.current_waypoint_idx += 1
                self.time_at_current_waypoint = 0.0
        elif self.grab_state == "trajectory3" and self.current_waypoint_idx < len(self.base_trajectory3) - 1:
            start_x, start_y, start_yaw = self.base_trajectory3[self.current_waypoint_idx]
            end_x, end_y, end_yaw = self.base_trajectory3[self.current_waypoint_idx + 1]
            ...
            if self.grab_state in ["trajectory", "lower", "hold", "return", "done"]:
                target_x, target_y, target_yaw = self.base_trajectory[-1]
            elif self.grab_state in ["trajectory2", "lower2", "hold2", "return2"]:
                idx = min(self.current_waypoint_idx, len(self.base_trajectory2) - 1)
                target_x, target_y, target_yaw = self.base_trajectory2[idx]
            else:
                idx = min(self.current_waypoint_idx, len(self.base_trajectory3) - 1)
                target_x, target_y, target_yaw = self.base_trajectory3[idx]
            self.progress = 1.0 if self.grab_state in ["hold", "hold2"] else self.progress
    ...
    def control_arm_trajectory(self):
        alpha = 0.05 
        act_offset = np.array([0, 0, -0.075428, -0.303811])
        
        if self.grab_state == "trajectory":
            if self.current_waypoint_idx < len(self.arm_trajectory) - 1:
                start_x, start_y, start_z = self.arm_trajectory[self.current_waypoint_idx]
                end_x, end_y, end_z = self.arm_trajectory[self.current_waypoint_idx + 1]
                
                target_x = start_x + (end_x - start_x) * self.progress
                target_y = start_y + (end_y - start_y) * self.progress
                target_z = start_z + (end_z - start_z) * self.progress
            else:
                target_x, target_y, target_z = self.arm_trajectory[-1]
                if self.progress >= 1.0:
                    self.grab_state = "lower"
                    self.grab_time = 0.0
        elif self.grab_state == "lower":
            start_x, start_y, start_z = self.arm_trajectory[-1]
            end_x, end_y, end_z = 0.6, 0.0, 1.1
            z_distance = abs(end_z - start_z)
            move_time = z_distance / self.grab_move_speed if z_distance > 0 else 1.0
            self.grab_time += self.model.opt.timestep
            progress = min(self.grab_time / move_time, 1.0) if z_distance > 0 else 1.0
            ...
            if progress >= 1.0:
                self.grab_state = "hold"
                self.grab_time = 0.0
        elif self.grab_state == "hold":
            target_x, target_y, target_z = 0.6, 0.0, 1.1
            ...
            self.grab_time += self.model.opt.timestep
            if self.grab_time >= self.grab_hold_duration:
                self.grab_state = "return"
                self.grab_time = 0.0
        elif self.grab_state == "return":
            start_x, start_y, start_z = 0.6, 0.0, 1.1
            end_x, end_y, end_z = self.arm_trajectory[-1]
            z_distance = abs(end_z - start_z)
            move_time = z_distance / self.grab_move_speed if z_distance > 0 else 1.0
            self.grab_time += self.model.opt.timestep
            progress = min(self.grab_time / move_time, 1.0) if z_distance > 0 else 1.0
            ...
            if progress >= 1.0:
                self.grab_state = "trajectory2"
                self.current_waypoint_idx = 0
                self.time_at_current_waypoint = 0.0
        elif self.grab_state == "trajectory2":
            if self.current_waypoint_idx < len(self.arm_trajectory2) - 1:
                start_x, start_y, start_z = self.arm_trajectory2[self.current_waypoint_idx]
                end_x, end_y, end_z = self.arm_trajectory2[self.current_waypoint_idx + 1]
                
                target_x = start_x + (end_x - start_x) * self.progress
                target_y = start_y + (end_y - start_y) * self.progress
                target_z = start_z + (end_z - start_z) * self.progress
            else:
                target_x, target_y, target_z = self.arm_trajectory2[-1]
                if self.progress >= 1.0:
                    self.grab_state = "lower2"
                    self.grab_time = 0.0
        elif self.grab_state == "lower2":
            start_x, start_y, start_z = self.arm_trajectory2[-2]
            end_x, end_y, end_z = self.arm_trajectory2[-1]
            z_distance = abs(end_z - start_z)
            if z_distance == 0:
                target_x, target_y, target_z = end_x, end_y, end_z
                self.grab_state = "hold2"
                self.grab_time = 0.0
            else:
                move_time = z_distance / self.grab_move_speed
                self.grab_time += self.model.opt.timestep
                progress = min(self.grab_time / move_time, 1.0)
                ...
                if progress >= 1.0:
                    self.grab_state = "hold2"
                    self.grab_time = 0.0
        elif self.grab_state == "hold2":
            target_x, target_y, target_z = 0.6, 0.0, 1.13
            for act_id in self.gripper_act_ids[0:3]:
                current_val = self.data.ctrl[act_id]
                new_val = np.clip(
                    current_val - self.GRIPPER_INCREMENT,
            ...
            self.grab_time += self.model.opt.timestep
            if self.grab_time >= self.grab_hold_duration:
                self.grab_state = "return2"
                self.grab_time = 0.0
        elif self.grab_state == "return2":
            start_x, start_y, start_z = 0.6, 0.0, 1.13
            end_x, end_y, end_z = 0.6, 0.0, 1.2
            z_distance = abs(end_z - start_z)
            move_time = z_distance / self.grab_move_speed if z_distance > 0 else 1.0
            self.grab_time += self.model.opt.timestep
            progress = min(self.grab_time / move_time, 1.0) if z_distance > 0 else 1.0
            ...
            if progress >= 1.0:
                self.grab_state = "trajectory3"
                self.current_waypoint_idx = 0
                self.time_at_current_waypoint = 0.0
        elif self.grab_state == "trajectory3":
            if self.current_waypoint_idx < len(self.arm_trajectory3) - 1:
                start_x, start_y, start_z = self.arm_trajectory3[self.current_waypoint_idx]
                end_x, end_y, end_z = self.arm_trajectory3[self.current_waypoint_idx + 1]
                
                target_x = start_x + (end_x - start_x) * self.progress
                target_y = start_y + (end_y - start_y) * self.progress
                target_z = start_z + (end_z - start_z) * self.progress
            else:
                target_x, target_y, target_z = self.arm_trajectory3[-1]
                if self.progress >= 1.0:
                    self.grab_state = "done3"
        else:
            target_x, target_y, target_z = self.arm_trajectory3[-1]

        local_target = [target_x, target_y, target_z]
        self.ik_solution(local_target=local_target)
